...
const validation = (keys, foodDb, food) => {
  ...
  var dietLabels = food.dietLabels;
  var arr;
  console.log(keys);
  for (var i = 0; i < keys.length; i++) {
    if (keys[i] == "calory") {
      calory !== undefined && calory !== ""
        ? (arr = foodDb.filter((obj) => obj.recipe.calories < food.calory))
        : (arr = foodDb.map((obj) => obj));
    }

    if (keys[i] == "dietLabels") {
      console.log("pass!!!!");
      if (dietLabels !== undefined && dietLabels !== "dietLabels") {
        arr = arr.filter((obj) =>
          searchType(dietLabels, obj.recipe.dietLabels)
        );
      }
    }
    if (keys[i] == "cuisineType") {
      ...
      if (cuisineType !== undefined && cuisineType !== "cuisineType") {
        arr = arr.filter((obj) =>
          searchType(cuisineType, obj.recipe.cuisineType)
        );
      }
    }
  }
